...
generate: install-controller-gen
	...
	@controller-gen crd rbac:roleName=metal-token-rotate webhook paths="./..." output:crd:artifacts:config=crd
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/ironcore-dev/metal-token-rotate\e[0m\n"
	@goimports -w -local github.com/ironcore-dev/metal-token-rotate $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
